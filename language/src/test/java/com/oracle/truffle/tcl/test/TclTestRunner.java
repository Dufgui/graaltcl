...
public class TclTestRunner extends ParentRunner<TestCase> {
    ...
    public TclTestRunner(Class<?> runningClass) throws InitializationError {
        super(runningClass);
        try {
    ...
    protected static List<TestCase> createTests(final Class<?> c) throws IOException, InitializationError {
        ...
        if (suite.testCaseDirectory() != TclTestSuite.class) {
    ...
    private static void run(Context context, Path path, PrintWriter out) throws IOException {
        try {
            ...
            Source source = Source.newBuilder(TclLanguage.ID, path.toFile()).interactive(true).build();
    ...
    public static void runInMain(Class<?> testClass, String[] args) throws InitializationError, NoTestsRemainException {
        ...
        TclTestRunner suite = new TclTestRunner(testClass);
        if (args.length > 0) {
