...
public class TclTestRunner
        extends
        ParentRunner<TestCase> {

    private static final String SOURCE_SUFFIX = ".tcl";
    private static final String INPUT_SUFFIX = ".input";
    private static final String OUTPUT_SUFFIX = ".output";

    private static final String LF = System
            .getProperty(
                    "line.separator");
    ...
    public TclTestRunner(
            Class<?> runningClass)
            throws InitializationError {
        super(runningClass);
        try {
            testCases = createTests(
                    runningClass);
        } catch (IOException e) {
    ...
    protected static List<TestCase> createTests(
            final Class<?> c)
            throws IOException,
            InitializationError {
        TclTestSuite suite = c
                .getAnnotation(
                        TclTestSuite.class);
        if (suite == null) {
            throw new InitializationError(
        ...
        if (suite
                .testCaseDirectory() != TclTestSuite.class) {
            testCaseDirectory = suite
                    .testCaseDirectory();
        }
        Path root = getRootViaResourceURL(
                testCaseDirectory,
                paths);

        if (root == null) {
            for (String path : paths) {
                Path candidate = FileSystems
                        .getDefault()
                        .getPath(
                                path);
                if (Files
                ...
                new SimpleFileVisitor<Path>() {
                    ...
                    public FileVisitResult visitFile(
                        ...
                        if (sourceName
                            ...
                            Path inputFile = sourceFile
                                    .resolveSibling(
                                            baseName + INPUT_SUFFIX);
                            String testInput = "";
                            if (Files
                                    .exists(inputFile)) {
                                testInput = readAllLines(
                                        inputFile);
                            }

                            Path outputFile = sourceFile
                                    .resolveSibling(
                                            baseName + OUTPUT_SUFFIX);
                            String expectedOutput = "";
                            if (Files
                                    .exists(outputFile)) {
                                expectedOutput = readAllLines(
                                        outputFile);
                            }
     ...
     */
    private static String explodeJarToTempDir(
            File jarfilePath) {
        try {
            final Path jarfileDir = Files
                    .createTempDirectory(
                            jarfilePath
                                    .getName());
            Runtime.getRuntime()
    ...
    public static Path getRootViaResourceURL(
            final Class<?> c,
            String[] paths) {
        URL url = c
                .getResource(
        ...
        if (url != null) {
            ...
            String externalForm = url
                    .toExternalForm();
            String classPart = sep
                    + c.getName()
                            .replace(
                                    '.',
                                    sep)
            ...
            if (externalForm
                    .startsWith(
                            "jar:file:")) {
                prefix = "jar:file:";
                int bang = externalForm
                        .indexOf(
                                '!',
                                prefix.length());
                Assume.assumeTrue(
                        bang != -1);
                File jarfilePath = new File(
                        externalForm
                                .substring(
                                        prefix.length(),
                                        bang));
    ...
    private static final List<NodeFactory<? extends TclBuiltinNode>> builtins = new ArrayList<>();
    ...
    protected void runChild(
        ...
        try {
            ...
            for (NodeFactory<? extends TclBuiltinNode> builtin : builtins) {
                TclLanguage
                        .installBuiltin(
                                builtin);
            }
                    ...
                    .in(new ByteArrayInputStream(
                            testCase.testInput
                                    .getBytes(
                                            "UTF-8")))
                    .out(out);
            for (Map.Entry<String, String> e : testCase.options
                    .entrySet()) {
                builder.option(
                        e.getKey(),
                        e.getValue());
    ...
    public static void runInMain(
        ...
        core.addListener(
                new TextListener(
                        System.out));
        TclTestRunner suite = new TclTestRunner(
                testClass);
        if (args.length > 0) {
            suite.filter(
                    new NameFilter(
