...
public class TclSharedCodeSeparatedEnvTest {
    ...
    public void initializeEngines() {
        ...
        int instances = TclLanguage.counter;
        // @formatter:off
        e1 = Context.newBuilder("tcl").engine(engine).out(os1).allowPolyglotAccess(PolyglotAccess.ALL).build();
        e1.getPolyglotBindings().putMember("extra", 1);
        e2 = Context.newBuilder("tcl").engine(engine).out(os2).allowPolyglotAccess(PolyglotAccess.ALL).build();
        e2.getPolyglotBindings().putMember("extra", 2);
        e1.initialize("tcl");
        e2.initialize("tcl");
        assertEquals("One TclLLanguage instance created", instances + 1, TclLanguage.counter);
    }
