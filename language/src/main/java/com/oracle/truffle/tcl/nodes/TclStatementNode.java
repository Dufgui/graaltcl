...
/**
 * The base class of all Truffle nodes for tcl. All nodes (even expressions) can
 * be used as statements, i.e., without returning a value. The
 * {@link VirtualFrame} provides access to the local variables.
 */
@NodeInfo(language = "tcl", description = "The abstract base node for all tcl statements")
@GenerateWrapper
public abstract class TclStatementNode
        extends
        TclScopedNode
        implements
        InstrumentableNode {
    ...
    public final SourceSection getSourceSection() {
        ...
        SourceSection rootSourceSection = rootNode
                .getSourceSection();
    ...
    public final void setSourceSection(
            int charIndex,
            int length) {
        assert sourceCharIndex == NO_SOURCE
                : "source must only be set once";
        if (charIndex < 0) {
    ...
    public WrapperNode createWrapper(
            ProbeNode probe) {
        return new TclStatementNodeWrapper(
                this,
                probe);
    }
     ...
     */
    public static String formatSourceSection(
            Node node) {
        if (node == null) {
            return "<unknown>";
        }
        SourceSection section = node
                .getSourceSection();
        ...
        if (section == null
            ...
            String sourceName = section
                    .getSource()
                    .getName();
            int startLine = section
                    .getStartLine();
            return String
                    .format("%s:%d%s",
                            sourceName,
