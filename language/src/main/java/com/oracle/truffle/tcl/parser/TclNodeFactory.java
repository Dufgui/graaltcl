...
public class TclNodeFactory {
    ...
    public void startFunction(
        ...
        functionStartPos = nameToken
                .getStartIndex();
        functionName = nameToken
                .getText();
        functionBodyStartPos = bodyStartToken
                .getStartIndex();
    ...
    public void addFormalParameter(
         ...
         */
        final TclReadArgumentNode readArg = new TclReadArgumentNode(
                parameterCount);
        readArg.setSourceSection(
                nameToken
                        .getStartIndex(),
                nameToken
                        .getText()
                        .length());
        TclExpressionNode assignment = createAssignment(
                createStringLiteral(
                        nameToken,
                        false),
                readArg,
    ...
    public void finishFunction(
        ...
        if (bodyNode == null) {
            ...
            methodNodes
                    .add(bodyNode);
            final int bodyEndPos = bodyNode
                    .getSourceEndIndex();
            final SourceSection functionSrc = source
                    .createSection(
                            functionStartPos,
                            bodyEndPos
                                    - functionStartPos);
            final TclStatementNode methodBlock = finishBlock(
                    methodNodes,
                    parameterCount,
                    ...
                    bodyEndPos
                            - functionBodyStartPos);
            assert lexicalScope == null
                    : "Wrong scoping of blocks in parser";

            final TclFunctionBodyNode functionBodyNode = new TclFunctionBodyNode(
                    methodBlock);
            functionBodyNode
                    .setSourceSection(
                            functionSrc
            ...
            final TclRootNode rootNode = new TclRootNode(
                    language,
                    frameDescriptor,
                    functionBodyNode,
                    functionSrc,
    ...
    public TclStatementNode finishBlock(
            List<TclStatementNode> bodyNodes,
            int startPos,
            int length) {
        return finishBlock(
                bodyNodes,
    ...
    public TclStatementNode finishBlock(
        ...
        List<TclStatementNode> flattenedNodes = new ArrayList<>(
                bodyNodes
                        .size());
        flattenBlocks(
                bodyNodes,
                flattenedNodes);
        int n = flattenedNodes
                .size();
        for (int i = skipCount; i < n; i++) {
            TclStatementNode statement = flattenedNodes
                    .get(i);
            if (statement
                    .hasSource()
                    && !isHaltInCondition(
        ...
        TclBlockNode blockNode = new TclBlockNode(
                flattenedNodes
                        .toArray(
                                new TclStatementNode[flattenedNodes
                                        .size()]));
        blockNode
                .setSourceSection(
                        startPos,
    ...
    private static boolean isHaltInCondition(
            TclStatementNode statement) {
        return (statement instanceof TclIfNode)
                || (statement instanceof TclWhileNode);
    }

    private void flattenBlocks(
            Iterable<? extends TclStatementNode> bodyNodes,
            List<TclStatementNode> flattenedNodes) {
     ...
     */
    public TclStatementNode createBreak(
            Token breakToken) {
     ...
     */
    public TclStatementNode createContinue(
            Token continueToken) {
     ...
     */
    public TclStatementNode createWhile(
            Token whileToken,
        ...
        conditionNode
                .addStatementTag();
        final int start = whileToken
                .getStartIndex();
        final int end = bodyNode
                .getSourceEndIndex();
        final TclWhileNode whileNode = new TclWhileNode(
                conditionNode,
                bodyNode);
        whileNode
                .setSourceSection(
     ...
     * @return An TclIfNode for the given parameters. null if either conditionNode or
     *         thenPartNode is null.
     */
    public TclStatementNode createIf(
            Token ifToken,
            TclExpressionNode conditionNode,
        ...
        conditionNode
                .addStatementTag();
        final int start = ifToken
                .getStartIndex();
        final int end = elsePartNode == null
                ? thenPartNode
                        .getSourceEndIndex()
                : elsePartNode
                        .getSourceEndIndex();
        final TclIfNode ifNode = new TclIfNode(
                conditionNode,
                thenPartNode,
                elsePartNode);
        ifNode.setSourceSection(
                start,
                end - start);
        return ifNode;
    }
     ...
     */
    public TclStatementNode createReturn(
            Token t,
            TclExpressionNode valueNode) {
        final int start = t
                .getStartIndex();
        final int length = valueNode == null
                ? t.getText()
                        .length()
                : valueNode
                        .getSourceEndIndex()
                        - start;
        final TclReturnNode returnNode = new TclReturnNode(
                valueNode);
        returnNode
                .setSourceSection(
                        start,
     ...
     */
    public TclExpressionNode createBinary(
            Token opToken,
            TclExpressionNode leftNode,
            TclExpressionNode rightNode) {
        if (leftNode == null
                || rightNode == null) {
            return null;
        }
        final TclExpressionNode leftUnboxed = TclUnboxNodeGen
                .create(leftNode);
        final TclExpressionNode rightUnboxed = TclUnboxNodeGen
                .create(rightNode);
        ...
        switch (opToken
                .getText()) {
            case "+":
                result = TclAddNodeGen
                        .create(leftUnboxed,
                                rightUnboxed);
                break;
            case "*":
                result = TclMulNodeGen
                        .create(leftUnboxed,
                                rightUnboxed);
                break;
            case "/":
                result = TclDivNodeGen
                        .create(leftUnboxed,
                                rightUnboxed);
                break;
            case "-":
                result = TclSubNodeGen
                        .create(leftUnboxed,
                                rightUnboxed);
                break;
            case "<":
                result = TclLessThanNodeGen
                        .create(leftUnboxed,
                                rightUnboxed);
                break;
            case "<=":
                result = TclLessOrEqualNodeGen
                        .create(leftUnboxed,
                                rightUnboxed);
                break;
            case ">":
                result = TclLogicalNotNodeGen
                        .create(TclLessOrEqualNodeGen
                                .create(leftUnboxed,
                                        rightUnboxed));
                break;
            case ">=":
                result = TclLogicalNotNodeGen
                        .create(TclLessThanNodeGen
                                .create(leftUnboxed,
                                        rightUnboxed));
                break;
            case "==":
                result = TclEqualNodeGen
                        .create(leftUnboxed,
                                rightUnboxed);
                break;
            case "!=":
                result = TclLogicalNotNodeGen
                        .create(TclEqualNodeGen
                                .create(leftUnboxed,
                                        rightUnboxed));
                break;
            case "&&":
                result = new TclLogicalAndNode(
                        leftUnboxed,
                        rightUnboxed);
                break;
            case "||":
                result = new TclLogicalOrNode(
                        leftUnboxed,
                        rightUnboxed);
                break;
            default:
    ...
    public void addModuleStatement(
     ...
     * @return An TclInvokeNode for the given parameters. null if functionNode or any
     *         of the parameterNodes are null.
     */
    public TclExpressionNode createCall(
            TclExpressionNode functionNode,
            List<TclExpressionNode> parameterNodes,
            Token finalToken) {
        ...
        final TclExpressionNode result = new TclInvokeNode(
                functionNode,
                parameterNodes
                        .toArray(
                                new TclExpressionNode[parameterNodes
                                        .size()]));

        final int startPos = functionNode
                .getSourceCharIndex();
        final int endPos = finalToken
                .getStartIndex()
                + finalToken
                        .getText()
                        .length();
     ...
     *         valueNode is null.
     */
    public TclExpressionNode createAssignment(
            TclExpressionNode nameNode,
            TclExpressionNode valueNode) {
    ...
    public TclExpressionNode createAssignment(
        ...
        String name = ((TclStringLiteralNode) nameNode)
                .executeGeneric(
                        null);
        ...
        FrameSlot existingSlot = lexicalScope.locals
                .put(name,
                        frameSlot);
        boolean newVariable = existingSlot == null;
        final TclExpressionNode result = TclWriteLocalVariableNodeGen
                .create(valueNode,
                        frameSlot,
                        nameNode,
                        newVariable);
        ...
        if (valueNode
                .hasSource()) {
            final int start = nameNode
                    .getSourceCharIndex();
            final int length = valueNode
                    .getSourceEndIndex()
                    - start;
            result.setSourceSection(
                    start,
     ...
     *         </ul>
     */
    public TclExpressionNode createRead(
            TclExpressionNode nameNode) {
        if (nameNode == null) {
            return null;
        }
        ...
        String name = ((TclStringLiteralNode) nameNode)
                .executeGeneric(
                        null);
        final TclExpressionNode result;
        final FrameSlot frameSlot = lexicalScope.locals
                .get(name);
        if (frameSlot != null) {
            /* Read of a local variable. */
            result = TclReadLocalVariableNodeGen
                    .create(frameSlot);
        } else {
            /*
             * Read of a global name. In our language, the only global names are functions.
             */
            result = new TclFunctionLiteralNode(
                    name);
        }
        result.setSourceSection(
    ...
    public TclExpressionNode createStringLiteral(
            Token literalToken,
        ...
        final TclStringLiteralNode result = new TclStringLiteralNode(
                literal.intern());
        srcFromToken(
                result,
                literalToken);
    ...
    public TclExpressionNode createIntegerLiteral(
            Token literalToken) {
        TclExpressionNode result;
        try {
            /* Try if the literal is small enough to fit into a long value. */
            result = new TclLongLiteralNode(
                    Long.parseLong(
                            literalToken
                                    .getText()));
        } catch (NumberFormatException ex) {
            /* Overflow of long value, so fall back to BigInteger. */
            result = new TclBigIntegerLiteralNode(
                    new BigInteger(
                            literalToken
                                    .getText()));
        }
    ...
    public TclExpressionNode createDoubleLiteral(
            Token literalToken) {
        TclExpressionNode result;
        try {
            /* Try if the literal is small enough to fit into a long value. */
            result = new TclDoubleLiteralNode(
                    Double.parseDouble(
                            literalToken
                                    .getText()));
        } catch (NumberFormatException ex) {
    ...
    public TclExpressionNode createParenExpression(
            TclExpressionNode expressionNode,
            int start,
            int length) {
        ...
        final TclParenExpressionNode result = new TclParenExpressionNode(
                expressionNode);
        result.setSourceSection(
                start,
                length);
     ...
     * @return An TclExpressionNode for the given parameters. null if receiverNode or
     *         nameNode is null.
     */
    public TclExpressionNode createReadProperty(
            TclExpressionNode receiverNode,
            TclExpressionNode nameNode) {
        ...
        final TclExpressionNode result = TclReadPropertyNodeGen
                .create(receiverNode,
                        nameNode);

        final int startPos = receiverNode
                .getSourceCharIndex();
        final int endPos = nameNode
                .getSourceEndIndex();
        result.setSourceSection(
                startPos,
                endPos - startPos);
     ...
     */
    public TclExpressionNode createWriteProperty(
            TclExpressionNode receiverNode,
            TclExpressionNode nameNode,
            TclExpressionNode valueNode) {
        if (receiverNode == null
        ...
        final TclExpressionNode result = TclWritePropertyNodeGen
                .create(receiverNode,
                        nameNode,
                        valueNode);

        final int start = receiverNode
                .getSourceCharIndex();
        final int length = valueNode
                .getSourceEndIndex()
                - start;
     ...
     */
    private static void srcFromToken(
            TclStatementNode node,
            Token token) {
        node.setSourceSection(
                token.getStartIndex(),
                token.getText()
                        .length());
    }
