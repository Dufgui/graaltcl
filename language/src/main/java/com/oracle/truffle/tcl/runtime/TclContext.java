...
public final class TclContext {
    ...
    public TclContext(
        ...
        this.allocationReporter = env
                .lookup(AllocationReporter.class);
        this.functionRegistry = new TclFunctionRegistry(
                language);
        installBuiltins();
    ...
    private void installBuiltins() {
        installBuiltin(
                TclReadlnBuiltinFactory
                        .getInstance());
        installBuiltin(
                TclPrintlnBuiltinFactory
                        .getInstance());
        installBuiltin(
                TclNanoTimeBuiltinFactory
                        .getInstance());
        installBuiltin(
                TclDefineFunctionBuiltinFactory
                        .getInstance());
        installBuiltin(
                TclStackTraceBuiltinFactory
                        .getInstance());
        installBuiltin(
                TclHelloEqualsWorldBuiltinFactory
                        .getInstance());
        installBuiltin(
                TclNewObjectBuiltinFactory
                        .getInstance());
        installBuiltin(
                TclEvalBuiltinFactory
                        .getInstance());
        installBuiltin(
                TclImportBuiltinFactory
                        .getInstance());
        installBuiltin(
                TclGetSizeBuiltinFactory
                        .getInstance());
        installBuiltin(
                TclHasSizeBuiltinFactory
                        .getInstance());
        installBuiltin(
                TclIsExecutableBuiltinFactory
                        .getInstance());
        installBuiltin(
                TclIsNullBuiltinFactory
                        .getInstance());
        installBuiltin(
                TclWrapPrimitiveBuiltinFactory
                        .getInstance());
        installBuiltin(
                TclTypeOfBuiltinFactory
                        .getInstance());
        installBuiltin(
                TclIsInstanceBuiltinFactory
                        .getInstance());
    }

    public void installBuiltin(
            NodeFactory<? extends TclBuiltinNode> factory) {
        /* Register the builtin function in our function registry. */
        RootCallTarget target = language
                .lookupBuiltin(
                        factory);
     ...
     */
    public static Object fromForeignValue(
            Object a) {
        if (a instanceof Long
                || a instanceof TclBigNumber
                || a instanceof String
                || a instanceof Boolean) {
            return a;
        } else
            if (a instanceof Character) {
                return fromForeignCharacter(
                        (Character) a);
            } else
                if (a instanceof Number) {
                    return fromForeignNumber(
                            a);
                } else
                    if (a instanceof TruffleObject) {
                        return a;
                    } else
                        if (a instanceof TclContext) {
                            return a;
                        }
        throw shouldNotReachHere(
                "Value is not a truffle value.");
    }

    @TruffleBoundary
    private static long fromForeignNumber(
            Object a) {
        return ((Number) a)
                .longValue();
    }
    ...
    @TruffleBoundary
    private static String fromForeignCharacter(
            char c) {
        return String
                .valueOf(
                        c);
