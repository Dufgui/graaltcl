...
/**
 * Represents a tcl function. On the Truffle level, a callable element is represented by a
 * {@link RootCallTarget call target}. This class encapsulates a call target, and adds version
 * support: functions in tcl can be redefined, i.e. changed at run time. When a function is
 ...
 * <p>
 * The {@link #callTarget} can be {@code null}. To ensure that only one {@link TclFunction} instance
 * per name exists, the {@link TclFunctionRegistry} creates an instance also when performing name
...
public final class TclFunction implements TruffleObject {
    ...
    /**
     * {@link TclFunction} instances are always visible as executable to other languages.
     */
    @SuppressWarnings("static-method")
    @ExportMessage
    @TruffleBoundary
    ...
    abstract static class Execute {
         ...
         *
         * <p>
         * Since tcl is a quite simple language, the benefit of the inline cache seems small: after
         ...
         * target is usually much more complicated than in tcl.
         * </p>
         *
         * <p>
         ...
         * function is redefined, the call target maintained by the TclFunction object is changed. To
         * avoid a check for that, we use an Assumption that is invalidated by the TclFunction when
