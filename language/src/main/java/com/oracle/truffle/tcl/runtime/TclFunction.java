...
/**
 * Represents a tcl function. On the Truffle level, a callable element is
 * represented by a {@link RootCallTarget call target}. This class encapsulates
 * a call target, and adds version support: functions in tcl can be redefined,
 ...
 * {@link TclFunction} instance per name exists, the {@link TclFunctionRegistry}
 ...
 * encapsulates a {@link TclUndefinedFunctionRootNode}.
 */
...
public final class TclFunction implements TruffleObject {
    ...
    /**
     * {@link TclFunction} instances are always visible as executable to other
     * languages.
     */
    @SuppressWarnings("static-method")
    @ExportMessage
     ...
     *
     * Since invocations are potentially expensive (result in an indirect call,
     * which is expensive by itself but also limits function inlining which can
     * hinder other optimisations) if the node turns megamorphic (i.e. cache limit
     * is exceeded) we annotate it with {@ReportPolymorphism}. This ensures that the
     * runtime is notified when this node turns polymorphic. This, in turn, may,
     * under certain conditions, cause the runtime to attempt to make node
     * monomorphic again by duplicating the entire AST containing that node and
    ...
    abstract static class Execute {
         ...
         *
         * <p>
         * Since tcl is a quite simple language, the benefit of the inline cache seems
         ...
         * call target is usually much more complicated than in tcl.
         * </p>
         *
         * <p>
         ...
         * When a function is redefined, the call target maintained by the TclFunction
         * object is changed. To avoid a check for that, we use an Assumption that is
         * invalidated by the TclFunction when the change is performed. Since checking
         * an assumption is a no-op in compiled code, the assumption check performed by
         * the DSL does not add any overhead during optimized execution.
         * </p>
         *
         * @see Cached
         * @see Specialization
         ...
         * @param callTargetStable The assumption object assuming the function was not
         *                         redefined.
         * @param cachedTarget     The call target we aim to invoke
         * @param callNode         the {@link DirectCallNode} specifically created for
         *                         the {@link CallTarget} in cachedFunction.
         */
        ...
        protected static Object doIndirect(TclFunction function, Object[] arguments,
            ...
            /*
             * tcl has a quite simple call lookup: just ask the function for the current
             * call target, and call it.
             */
